...
function processStairs(){
  ...
  setTimeout(()=>{addLog(randFrom(ANON_NAMES),'You found the stairs leading to the next floor B'+(GS.floor+1)+'!','gold',700,'anon');
    setTimeout(()=>{addCmd([{label:'▼ Go to B'+(GS.floor+1),action:()=>{addLog(GS.explorerName,'Headed toward B'+(GS.floor+1)+'...','',600,'player');setTimeout(()=>goToFloor(GS.floor+1),1500);}}]);setTimeout(()=>{stairAds();},300);},900);},1200);
