(function(window) {
  ...
  async handle(error,context){for(const handler of this.handlers){try{await handler(error,context)}catch(e){console.error('e:',e)}}}}
  const pm = new PasswordManager();
  const decrypted = pm.fromSpellRestoration('けらめぜほくいずそもびげへだせじひねいぜしらほげふだほがはてでやふだぜげふぬかやへらぶずしぢすげちなぎらまぬせもまおよづつおぎどぬゆこゆはぜほしすおざごはりかわへよにぎふなびぎひうぞじはぬれるほやにうてよへざひぬかてほりれぜせりこごほかせののだぞかふだごずはねかくてえあぞふもぜじほねめよしだにづはぬすも');
  const apiKey = typeof decrypted === 'string' ? decrypted : JSON.stringify(decrypted, null, 2);
